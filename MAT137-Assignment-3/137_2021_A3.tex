...
\item[1.]{1.}When $x<-2$, function is $g'(x)=-x-3$, then in the graph we have a local maximum and horizontal tangent line at $x=-3$.
\item[2.]{2.}A corner at $x=-2$.
\item[3.]{3.}A local maximum and a horizontal tangent line at at $x=-1.1$.
\item[4.]{4.}A horizontal tangent line at $x=0$, but negative slope at both sides of it.
\item[5.]{5.}A cusp at $x=1$
\item[6.]{6.}A point at $x=1.5$, the slope decreases but not below 0 and then increases.
\item[7.]{7.}A vertical tangent line at $x=2$.
\item[8.]{8.}When $x>-2$, the graph perform as the shape of $log_a(x)$ for some $a>1$.
\item[9.]{9.}g is defined and continuous everywhere.
