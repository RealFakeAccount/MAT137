...
\begin{itemize}
	...
	\begin{itemize}
		...
		Therefore, $\sum_{n = 1}^{\infty} (-1)^n \frac{1}{n}$ is convergent.
	\end{itemize}
	\item Since when $x = 1$ and $x=-1$, $h(1)=h(-1)=\sum_{n = 1}^{\infty} (-1)^n\frac{1}{n}$ and it is convergent.
	\begin{itemize}
	    ...
	    \item When we denote $a_n=(-1)^n\frac{1}{n}$:
            ...
            We have shown $a_n$ is convergent when $x=1$ and $x=-1$.
...
Therefore, $h(x)$ is convergent when $|x| \leq 1$ as $x\in[-1,1]$ and conditionally convergent for $x=1$ and $x=-1$ (two different points). \blacksquare
