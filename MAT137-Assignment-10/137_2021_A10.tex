...
\end{align*}
Since the Maclaurin series of $f$ is unique, so two equation \circled{1} and \circled{2} must be the same. Then we will have the same coefficient of $x^{137}$ in both \circled{1} and \circled{2} and we use $n_1$ to replace $n$ in $\sum_{n=0}^\infty \frac{2^n\cdot x^{5n}}{n!}$ and we use $n_2$ to replace $n$ in $\sum_{n=0}^\infty  (-1)^n\frac{x^{4n}}{(2n)!}$ then we can get two new equation:
\begin{align*}
