...
We can easily proof (or just observe the picture) that the domain of $f$ is $\R$ and $f$ is differentiable.

For the property(c), since $f$ oscillate faster as x increase, from the definition of derivative, we know that
$\forall b \in \R, \exists x > 0, f(x) = 0 \land f'(x) > b$ and $f'(x) \neq 0$ , using the theorem of derivative of the inverse of a function $(f^{-1})'(y) = \frac{1}{f'(x)}$, 
we can assume that $\forall c > 0, \exists x > 0, 0 < g'(x) < c$.
