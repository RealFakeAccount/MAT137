...
\item  \label{likes}  In this problem, assume all functions have domain $\R$.  I will define a new concept.  For every pair of functions $f$ and $g$, we define the set
	...
	\begin{enumerate}
		...
		We can get
		$$
		x < (x + \pi) \in \{x \in \R : \; 2k\pi + \pi < x < (2k + 1)\pi + \pi\ for\ some\ k\in\Z\} \\
		$$
		$$
		 = \{X \in \R : \; (2k + 1)\pi < x < (2k + 2)\pi\ for\ some\ k\in\Z\}
		$$
		$$
		\exists y \in \{y \in \R : \; (2k + 1)\pi < y < (2k + 2)\pi\ for\ some\ k\in\Z\} \; S.T. \; y=x + \pi > x \quad \quad \blacksquare
		$$
		...
		we can get $\M{f}{g} = \{x \in \R : \;  x > 3 \}, \ \M{g}{f} = \{y \in \R : \;  y < 3 \}$.\\
		Take $x=5 \in \M{f}{g}= \{x \in \R : \;  x > 3 \}$, Let$\ y \in \M{g}{f} = \{y \in \R : \;  y < 3 \}$, \\
		$x = 5 > 3 > y \quad \quad \blacksquare $
		\\
...
\item We continue with the assumptions, notation and definitions as in Question \ref{likes}.
	...
	\begin{enumerate}
		...
		$$
			\forall x \in \{x \in \R : tan(x) < t\ for\ some\ t\in\R\},\\
			\exists y \in \{y \in \R : t < tan(y)\ for\ some\ t\in\R\}\ S.T. \; x < y
		$$

		We know that if $x \in \R$, the range of $tan(x) \mbox{ is from } -\infty \mbox{ to } +\infty$\\
		So $\forall x \in \{x \in \R : tan(x) < t\ for\ some\ t\in\R\}, $ \\
